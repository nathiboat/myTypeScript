...
export default class GetReportQueue
{
        ...
        try {
            ...
            if(pendingTickets.length === 0)
            {
                ...
                let log = StaffLog.build({
                    staffId: staffId,
                    eventCode: `WALI_${ taskType.toUpperCase().split('-').join('_') }_VIEW`,
    ...
    private async fillQueueCallback(taskType : string) : Promise<MemberReport>
    {
